...
async function main() {
  ...
  // Seed AI Agents
  const aggressive = await prisma.aiAgent.upsert({
    where: { name: 'Aggressive' },
    update: {},
    create: {
      name: 'Aggressive',
      description: 'High-conviction momentum and event-driven strategy',
      riskProfile: 'HIGH',
      promptTemplate: `You are an aggressive investment agent focused on capturing momentum and event-driven opportunities.
...
- Cite specific metrics (momentum %, sentiment score, TA signals)`,
  ...
  const balanced = await prisma.aiAgent.upsert({
    where: { name: 'Balanced' },
    update: {},
    create: {
      name: 'Balanced',
      description: 'Diversified value-momentum blend with moderate risk',
      riskProfile: 'MODERATE',
      promptTemplate: `You are a balanced investment agent seeking optimal risk-adjusted returns.
...
- Cite factor scores, regime context, and diversification`,
  ...
  const defensive = await prisma.aiAgent.upsert({
    where: { name: 'Defensive' },
    update: {},
    create: {
      name: 'Defensive',
      description: 'Capital preservation with quality and low-volatility focus',
      riskProfile: 'LOW',
      promptTemplate: `You are a defensive investment agent prioritizing capital preservation.
...
- Cite quality scores, risk metrics, and regime considerations`,
  ...
  console.log('✅ AI Agents seeded:', { aggressive, balanced, defensive });

  // Seed sample macro series
  const cpiSeries = await prisma.macroSeries.upsert({
    where: { code: 'CPIAUCSL' },
    update: {},
    create: {
      code: 'CPIAUCSL',
  ...
  const unrateSeries = await prisma.macroSeries.upsert({
    where: { code: 'UNRATE' },
    update: {},
    create: {
      code: 'UNRATE',
  ...
  console.log('✅ Macro series seeded:', { cpiSeries, unrateSeries });
